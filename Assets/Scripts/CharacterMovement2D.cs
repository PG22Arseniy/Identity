...
public class CharacterMovement2D : CharacterMovement
{
    ...
    private bool CheckGrounded()
    {
        ...
        // avoid jumping on platform from uncollidable layer 
        switch (LayerMask.LayerToName(gameObject.layer))
        {
            case "BlackCharacter":
                if (firstHit.transform.gameObject.layer == LayerMask.NameToLayer("WhiteObject"))
                    return false;
                break;
            case "WhiteCharacter":
                if (firstHit.transform.gameObject.layer == LayerMask.NameToLayer("BlackObject"))
                    return false;
                break;
        }  
